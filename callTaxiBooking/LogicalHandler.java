...
public enum  LogicalHandler {
   ...
   /* private List<Taxi> getNearestTaxis(List<Taxi> freeTaxis,Customer customer)
    {
        List<Taxi> nearestTaxis=new ArrayList<>();
        int min=999;
    ...
    private void bookingTaxi(List<Taxi> freeTaxis,Customer customer)
    {
       ...
       {
           int distanceBetweenCustomerAndTaxi=((list.getCurrentSpot()-'0')-(customer.getPickUpPoint()-'0'))*15;
           if (distanceBetweenCustomerAndTaxi<=min&&current<=list.getEarnings())
           {
               current= list.getEarnings();
               totalEarnings= list.getEarnings();
               min=Math.abs(distanceBetweenCustomerAndTaxi);
